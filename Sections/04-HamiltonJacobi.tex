...
\end{align*}
Sappiamo anche che $P_i = \alpha_i$ è una costante di moto, quindi possiamo riscrivere $S$ come 
\begin{equation*}
    S(\vq,\vP,t) = S(\vq,\alpha_1, \dots, \alpha_n, t)
\end{equation*}
e la trasformazione canonica diventa
\begin{equation*}
    \begin{cases}
        p_i = \pdv{S}{q_i} \\
    ...
    \end{cases}
\end{equation*}
Riscrivendo quindi l'equazione \eqref{eq:hamiltonjacobi-1} ottenendo
\begin{equation*}
    H\left( q_1, \dots, q_n, \pdv{S}{q_1}, \dots, \pdv{S}{q_n}, t \right) + \pdv{S}{t} = 0,
\end{equation*}
la quale viene detta \emph{equazione di Hamilton--Jacobi}.

\subsection{Soluzioni dell'equazoni di Hamilton--Jacobi}

A differenza delle equazioni di Hamilton, l'equazione che abbiamo ricavato è un'equazione differenziale alle derivate parziali di primo ordine nella funzione incognita $S$. Il problema quindi diventa trovare soluzioni, ricordando che la soluzione generale di un'equazione differenziale alle derivate parziali dipende da funzioni arbitrarie.

Partiamo scrivendo l'equazione di Hamilton--Jacobi
\begin{equation*}
...
\begin{remark}
    Una soluzione completa dell'equazione di Hamilton--Jacobi non è la soluzione generale di tale equazione.
\end{remark}


\textbf{RIVEDERE}
Nel caso generale di equazioni di questo tipo si ha una cosa del tipo
\begin{equation*}
    F(x_1, \dots, x_{n+1}, \pdv{f}{x_1}, \dots, \pdv{f}{x_{n+1}}) = 0,
\end{equation*}
giustificando la necessità di $n+1$ costanti arbitrarie. Nel nostro caso il tempo non è una variabile come le altre nell'hamiltoniana e la $(n+1)$-esima costante è una costante additiva, totalmente trascurabile.
...
Si dimostra che è perfettamente legittimo scegliere $\gamma$ come una delle costanti arbitrarie $\alpha_i$, ad esempio $\gamma = \alpha_1$. In questo modo si ha
\begin{equation*}
