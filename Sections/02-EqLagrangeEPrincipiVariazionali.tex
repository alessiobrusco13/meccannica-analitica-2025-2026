...
Nell'ipotesi di vincoli ideali, cioé con $\var{L^{(v)}} \geq 0$\note{Le equazioni di Lagrange possono essere ricavate anche senza questa ipotesi attraverso il metodo dei moltiplicatori}, il principio si riduce alla disuguaglianza
\begin{equation*}
    \var{L^{(a)}} + \var{L^{(m)}} \leq 0.
\end{equation*}
Nel caso in cui i vincoli ideali siano anche bilaterali, cioé con  $\var{L^{(v)}} = 0$, il principio assume la forma di uguaglianza
\begin{equation*}
    \var{L^{(a)}} + \var{L^{(m)}} = 0.
\end{equation*}
...
Dalla forma dell'energia cinetica, i termini $T_2$, $T_1$ e $T_0$ sono forme rispettivamente quadratiche, lineari e indipendenti dalle velocità generalizzate $\dot{\vq}$. Essi costituiscono pertanto funzioni omogenee di grado 2, 1 e 0 rispetto alle $\dot{\vq}$. Possiamo allora valutare le derivate ricorrendo al seguente teorema.
