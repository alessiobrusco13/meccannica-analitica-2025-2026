...
Definiamo la variazione infinitesima \note{o perturbazione} di $y^*(x)$ come
\begin{align*}
...
\begin{proof}
    ...
    \begin{equation*}
        \eta(x) = \begin{cases}
            ...
            (x-\xi_1)^2(\xi_2-x)^2, \quad &x\in [\xi_1, \xi_2]\note{moltiplicando la potenza per $n$ è possibile dimostrare il lemam anche con funzioni $C^n$}
        \end{cases}.
    \end{equation*}
...
\end{equation*}
Possiamo quindi ottenere il seguente sviluppo di $I[y^* + \var{y}]$:
\begin{align*}
    I[y^* + \var{y}] &= I[y^*] + \alpha \int_{x_0}^{x_1} \left[\pdv{F}{y}\eta - \dv{x} \pdv{F}{y'}\eta'\right]  \dd{x} \dots.
\end{align*}
...
\begin{theorem}[Principio di Hamilton]\note{(1834)}
    Sia $\mathcal{S}$ un sistema olonomo di coordinate $\vq$ e lagrangiana $L$, $P_0$ e $P_1$ due configurazioni assunte da $\mathcal{S}$ agli istanti $t_0$ e $t_1$, rispettivamente. Tra le possibili traiettorie che collegano $P_0$ a $P_1$\note{$P_0$ e $P_1$ sono punti dello spazio delle configurazioni} in un intervallo di tempo $[t_0, t_1]$, il modo effettivo del sistema è quello in corrispondenza della traiettoria che rende stazionario il funzionale
    \begin{equation*}
        A = \int_{t_0}^{t_1} L(\vq, \vq', t) \dd{t},\note{Solitamente si indica con $S$}
    \end{equation*}
