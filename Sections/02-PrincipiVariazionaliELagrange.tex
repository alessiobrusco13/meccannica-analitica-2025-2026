...
Consideriamo un sistema di lagrangiana $\LL(\vq, \dot{\vq}, t)$ e un moto del sistema fissando $\vq(t)$ e $\dot{\vq}(t)$. Calcoliamo la derivata totale di $\LL$ rispetto al tempo:
\begin{equation*}
...
\begin{align*}
    \dv{\LL}{t} &= \sum_{h=1}^n \pdv{\LL}{q_h}\,\dot{q}_h + \dv{t}\sum_{h=1}^n \pdv{\LL}{\dot{q}_h}\,\dot{q}_h - \sum_{h=1}^n \dv{t}\pdv{\LL}{\dot{q}_h}\,\dot{q}_h + \pdv{\LL}{t} \\
...
\begin{theorem}[di Noether]
    \note{del 1918}
    Sia $f(\vq, s)$ un gruppo di trasformazioni ammissibili per la lagrangiana $\LL$ di un sistema. Allora, la funzione
    \begin{equation*}
