...
\subsection{Formulazione variazionale e principio di Hamilton}
...
\begin{definition}[Funzionali integrali del calcolo delle variazioni]
    ...
    La funzione $F$ è detta \emph{lagrangiana} per analogia con il caso meccanico.
\end{definition}

Per parlare rigorosamente di estremi relativi, occorre specificare sia il significato di massimo/minimo sia il concetto di vicinanza tra funzioni.

Consideriamo il caso di un massimo. Per analogia con il caso di una funzione reale di una variabile, diremo che $I[y]$ è massimo in $y^*(x)$ se, per ogni funzione $y(x)$ sufficientemente vicina a $y^*(x)$, si ha
\begin{equation*}
    I[y] \leq I[y^*].
\end{equation*}

Definiamo la variazione infinitesima (o perturbazione) di $y^*(x)$ come
\begin{align*}
...
\begin{definition}[Estremi relativi di un funzionale]
    ...
    \begin{align*}
        I[y^*] &\geq I[y^* + \alpha \eta], \quad \forall \alpha \in (-\varepsilon, \varepsilon), \forall \eta.
    \end{align*}
    Analogamente, diremo che $y^*(x)$ è un minimo relativo se
    \begin{align*}
        I[y^*] &\leq I[y^* + \alpha \eta], \quad \forall \alpha \in (-\varepsilon, \varepsilon), \forall \eta.
    \end{align*}
\end{definition}

\begin{theorem}[di Eulero--Lagrange]
